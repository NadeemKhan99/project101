    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {            
            var headers = {
                    ...
                    axios.post(
                        'http://localhost/back_end/signup.php',values, headers
                    ).then(
                        res =>{
                            setcheck(false);
                            alert(res.data.id)
        ...
        validationSchema: yup.object({
            fname: yup.string().matches("^[a-zA-Z]{1,}[^0-9]$", "Invalid name, only use text").required("This field is required!"),
            lname: yup.string().matches("^[a-zA-Z]{1,}[^0-9]$", "Invalid name, only use text").required("This field is required!"),
        ...
        <div className="doctor_form">
            <form onSubmit={formik.handleSubmit}>
                ...
                <div className="mb-2 p-2">
                    ...
                    <input type="text" className="form-control" values={formik.values.fname} onChange={formik.handleChange} autoFocus id="fname" placeholder="Enter first name" />
                    {formik.errors.fname? <div className="error">{formik.errors.fname}</div>: ""}
                </div>

                <div className="mb-2 p-2">
                    <label htmlFor="lname" className="form-label">Last Name</label>
                    <input type="text" className="form-control" values={formik.values.lname} onChange={formik.handleChange} id="lname" placeholder="Enter last name" />
